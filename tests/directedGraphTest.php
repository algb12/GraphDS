...
class DirectedGraphTest extends PHPUnit_Framework_TestCase
{
    public function testVertexAddRemove()
    {
        ...
        $this->assertContains('C', $g->vertices['A']->getOutNeighbors());
        $g->removeVertex('C');
        $this->assertArrayNotHasKey('C', $g->vertices);
        $this->assertArrayNotHasKey('C', $g->edges['A']);
        $this->assertNotContains('C', $g->vertices['A']->getOutNeighbors());
        $g->removeVertex('B');
    ...
    public function testEdgeAddRemove()
    {
        ...
        $g->addVertex('B');
        $g->addEdge('A', 'B');
        $this->assertEquals(true, isset($g->edges['A']['B']));
        $this->assertEquals(false, isset($g->edges['B']['A']));
        $this->assertContains('B', $g->vertices['A']->getOutNeighbors());
        $this->assertContains('A', $g->vertices['B']->getInNeighbors());
        $this->assertNotContains('B', $g->vertices['A']->getInNeighbors());
        $this->assertNotContains('A', $g->vertices['B']->getOutNeighbors());
        $g->removeEdge('A', 'B');
        $this->assertEquals(false, isset($g->edges['A']['B']));
        $this->assertEquals(false, isset($g->edges['B']['A']));
        $this->assertNotContains('B', $g->vertices['A']->getOutNeighbors());
        $this->assertNotContains('A', $g->vertices['B']->getInNeighbors());
    }
    ...
    public function testEdgeGetSetValue()
    {
        ...
        $g->addEdge('A', 'C');
        $g->addEdge('C', 'A');
        $g->edges['A']['B']->setValue(1.0);
        $g->edges['A']['C']->setValue(1.1);
        $g->edges['C']['A']->setValue(2);
        $this->assertEquals('1.0', $g->edges['A']['B']->getValue());
        $this->assertEquals('1.1', $g->edges['A']['C']->getValue());
        $this->assertEquals('2', $g->edges['C']['A']->getValue());
    }
    ...
    public function testVertexAdjacencyMethods()
    {
        ...
        $g->addEdge('A', 'C');
        $g->addEdge('C', 'B');
        $this->assertEquals(true, $g->vertices['A']->outAdjacent('B'));
        $this->assertEquals(false, $g->vertices['B']->outAdjacent('A'));
        $this->assertEquals(true, $g->vertices['C']->inAdjacent('A'));
        $this->assertEquals(false, $g->vertices['A']->inAdjacent('C'));
    ...
    public function testIndegreeAndOutdegree()
    {
        ...
        $g->addEdge('A', 'C');
        $g->addEdge('C', 'A');
        $this->assertEquals(1, $g->vertices['A']->getIndegree());
        $this->assertEquals(2, $g->vertices['A']->getOutdegree());
        $this->assertEquals(1, $g->vertices['B']->getIndegree());
        $this->assertEquals(0, $g->vertices['B']->getOutdegree());
        $this->assertEquals(1, $g->vertices['C']->getIndegree());
        $this->assertEquals(1, $g->vertices['C']->getOutdegree());
    }
    ...
    public function testTranspose()
    {
        ...
        $this->assertEquals($g, $h);
        $this->assertNull($g->edge('B', 'A'));
        $this->assertNotNull($g->edge('A', 'B'));
        $this->assertEquals(1, $g->edge('A', 'B')->getValue());
        $this->assertNull($i->edge('A', 'B'));
        $this->assertNotNull($i->edge('B', 'A'));
        $this->assertEquals(1, $i->edge('B', 'A')->getValue());
    }
